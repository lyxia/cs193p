...
//        }
    ...
    open var requestForOlder: Request? {
        if min_id == nil {
    ...
    open var requestForNewer: Request? {
        if max_id == nil {
    ...
    func performTwitterRequest(_ method: SLRequestMethod, handler: @escaping (PropertyList?) -> Void) {
        ...
        let request = SLRequest(
            ...
            url: URL(string: "\(Constants.TwitterURLPrefix)\(self.requestType)\(jsonExtension)"),
