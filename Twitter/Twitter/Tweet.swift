...
open class Tweet : NSObject
{
    ...
    open let hashtags: [Mention]
    open let urls: [Mention]
    open let userMentions: [Mention]
    
    open override var description: String { return "\(user) - \(created)\n\(text)\nhashtags: \(hashtags)\nurls: \(urls)\nuser_mentions: \(userMentions)" + "\nid: \(id)" }
...
open class Mention: NSObject
{
    open let keyword: String              // will include # or @ or http prefix
    open let nsrange: NSRange             // index into an NS[Attributed]String made from the Tweet's text
    
    open override var description: String { return "\(keyword) (\(nsrange.location), \(nsrange.location+nsrange.length-1))" }
